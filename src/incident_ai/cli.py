...
def build_parser() -> argparse.ArgumentParser:
    ...
    analyze.add_argument("source", nargs="?", default="-", help="Log file to analyze, or '-' to read from stdin (default).")
    analyze.add_argument("--json", action="store_true", help="Emit JSON output.")
    analyze.add_argument("--compact", action="store_true", help="Emit compact JSON. Implies --json.")
    analyze.add_argument("--sarif", action="store_true", help="Emit SARIF 2.1.0 for CI and security tooling.")
    analyze.add_argument("--all", action="store_true", help="Return every distinct recognized incident, ordered by confidence.")
    ...
    analyze.add_argument("--webhook", help="POST the structured analysis to this explicit HTTP(S) webhook; requires --redact.")
    analyze.add_argument(
        "--webhook-retries",
        type=int,
        default=0,
    ...
    analyze.add_argument("--enrich", action="store_true", help="Opt in to remote OpenAI enrichment after local analysis and redaction.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.group_by:
        ...
        _send_webhook_or_exit(parser, webhook_payload, args.webhook, redact=args.redact, max_retries=args.webhook_retries)
        if args.sarif:
    ...
    _send_webhook_or_exit(parser, analysis.to_dict(), args.webhook, redact=args.redact, max_retries=args.webhook_retries)
